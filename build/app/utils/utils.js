...
export const generateHeaderPDF = (invoiceData, doc) => {
    doc
...
export const generateFooterPDF = (doc) => {
    doc
        .fontSize(10)
...
export const generateCustomerInformationPDF = (doc, invoice) => {
    ...
    doc
        ...
        .text(invoice.recepter.identification + ', ' + 'Estado Sucre' + ', ' + 'Venezuela', 300, customerInformationTop + 30)
        .moveDown();
    generateHr(doc, 252);
};
export const generateTableRow = (doc, y, item, description, unitCost, quantity, lineTotal) => {
    doc
        .fontSize(10)
        .text(item, 50, y)
...
export const generateInvoiceTablePDF = (doc, invoice) => {
    let i;
    ...
    for (i = 0; i < invoice.products.length; i++) {
        ...
        generateTableRow(doc, position, item.code, item.description, item.unit_price.toString(), item.quantity.toString(), item.amount.toString());
        generateHr(doc, position + 20);
    }
    const subtotalPosition = invoiceTableTop + (i + 1) * 30;
    generateTableRow(doc, subtotalPosition, '', '', 'Subtotal', '', invoice.subtotal.toString());
    const paidToDatePosition = subtotalPosition + 20;
    generateTableRow(doc, paidToDatePosition, '', '', 'Pagado Hasta la Fecha', '', invoice.total.toString());
